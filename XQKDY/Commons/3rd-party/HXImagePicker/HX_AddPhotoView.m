...
- (instancetype)initWithMaxPhotoNum:(NSInteger)num WithSelectType:(SelectType)type viewController:(UIViewController *)controller
{
    if (self = [super init]) {
        self.maxNum = num;
        self.type = type;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(goAddPhotoVC) name:@"ChoosePhoto" object:nil];
...
- (void)goAddPhotoVC
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"photoChoose" object:nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 0) {
        ...
        BaseNavigation *nav = [[BaseNavigation alloc] initWithRootViewController:photo];
        [_currentController presentViewController:nav animated:YES completion:nil];
    }
}
