...
@interface messageCell ()<UIGestureRecognizerDelegate> {
...
- (void)setLayout:(MessageLisetLayout *)layout {
    if (layout) {
        _layout = layout;
        self.contentLabel.text = layout.phone;
        self.dateLabel.text = layout.date;
        self.titleLabel.text = layout.title;
        self.infoLabel.text = layout.content;
        self.messageBtn.selected = layout.isSelected;
        [self.copysBtn setTitle:layout.creatdate forState:0];
        if ([layout.title isEqualToString:@"上门取件"]) {
            [_iconImageView setImage:[UIImage imageNamed:@"icon_shou_"]];
        }
        else if ([layout.title isEqualToString:@"上门派件"])
        {
             [_iconImageView setImage:[UIImage imageNamed:@"icon_pai_"]];
        }
        else if ([layout.title isEqualToString:@"投诉任务"])
        {
            [_iconImageView setImage:[UIImage imageNamed:@"icon_su_"]];
        }
...
- (void)setupSwipeViewWithSwipeVelocityX:(CGFloat)velocityX {
    if (self.overlayerContentView == nil) {
        if (self.closeOtherCellSwipeViewWhenOpenSwipeView) {
            for (UITableViewCell *cell in [self.tableView visibleCells]) {
                if ([cell isKindOfClass:[messageCell class]]) {
                    messageCell *swipeCell = (messageCell *)cell;
                    if (swipeCell != self) {
        ...
        if (self.snapView == nil) {
            ...
            self.snapView.backgroundColor = [UIColor redColor];
            // 添加到cell上
