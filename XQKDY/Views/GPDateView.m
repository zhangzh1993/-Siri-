...
- (void)getSelectDistrictName
{
    ...
    _CodeStr = [self.districtArray[[_pickerView selectedRowInComponent:2]] objectForKey:Key_DivisionCode];
    _titleLabel.text = [NSString stringWithFormat:@"%@%@%@",self.provinceStr,self.cityStr,self.districtStr];
    NSLog( @"~~~~~~~_code:%@",_CodeStr);
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    if (component==0)
    {
        ...
        myView.text = [ProvinceDict objectForKey:Key_DivisionName];
        
    }
    else if(component ==1)
    {
        NSDictionary *CityDict = self.CityArray[row];
        myView.text = [CityDict objectForKey:Key_DivisionName];
    }
    else
    {
        NSDictionary *DistrictDict = self.districtArray[row];
        myView.text = [DistrictDict objectForKey:Key_DivisionName];
    }
